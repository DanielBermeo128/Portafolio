...
public class AgregarFondoController {
    ...
    @FXML
    private TextField capitalInicial;
    ...
    public void finalizadoFondo(ActionEvent event) {
    	...
    	if (action.get() == ButtonType.OK) {
				...
				capitalIni = capitalInicial.getText();
	    		...
	    		if(nombre=="" || comprobar.sonEspacios(nombre)) {
    				...
    				alertaError.showAndWait();
    				alerta.close();
	    		}else if(comprobar.noEsNumerica(capitalIni)){
	    			Alert alertaError = new Alert(AlertType.ERROR);
    				alertaError.setContentText("El capital inicial es invalido");
    				alertaError.setTitle("Error en capital inicial");
    				alertaError.showAndWait();
    				alerta.close();
	    		...
	    		if(insercionLista) {
	    			Fondos fondoAinsertar = new Fondos(nombre, Integer.parseInt(montoDeseado), Integer.parseInt(capitalIni));
	    			try {
						Conexion cn = new Conexion();
						cn.insertarFondo(fondoAinsertar);
